...
function toggleExamSection(sectionId) {
  const body = document.getElementById(`esbody-${sectionId}`);
  const chev = document.getElementById(`esbchev-${sectionId}`);
  const banner = document.getElementById(`esb-${sectionId}`);
  if (!body) return;

  const isOpen = openExamSections.has(sectionId);

  if (!isOpen) {
    body.classList.add('open');
    chev?.classList.add('open');
    banner?.classList.add('active');
    openExamSections.add(sectionId);
  } else {
    body.classList.remove('open');
    chev?.classList.remove('open');
    banner?.classList.remove('active');
    openExamSections.delete(sectionId);
  }
}
